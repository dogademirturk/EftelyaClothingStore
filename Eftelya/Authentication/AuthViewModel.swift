...
class AuthViewModel: ObservableObject {
    ...
    enum AuthenticationError: Error, Identifiable {
            ...
            var message: String {
                switch self {
                case .emailAlreadyInUse(let message), .weakPassword(let message), .unknownError(let message), .invalidEmail(let message):
                    return message
                }
            }
    ...
    func signIn(withEmail email: String, password: String) async throws {
        do {
            ...
            self.error = .unknownError(message: "There was an issue with logging in. Please check your credentials and try again. If the problem persists, please try again later.")
        }
    }
